...
const NAV = [
  { bn: 'সেবাসমূহ',     en: 'Services',    section: 'services'   },
  { bn: 'দৃষ্টি পরীক্ষা',en: 'Vision Test', section: 'vision-test'},
  { bn: 'বিশেষজ্ঞ',      en: 'Specialists', section: 'doctors'    },
  { bn: 'শাখা',          en: 'Branches',    section: 'branches', hasDrop: true },
];
...
export default function Navbar({ onNavigate, activeSection, lang, setLang }: NavbarProps) {
  ...
  const lbl = (item: typeof NAV[0]) => lang === 'bn' ? item.bn : item.en;

  return (
    <>
      {/* ── Top bar ──────────────────────────────────────────────────── */}
      <div style={{ background: 'var(--brand-navy)', color: 'rgba(255,255,255,0.72)', fontSize: '0.72rem', height: 32, display: 'flex', alignItems: 'center' }} className="hidden-mobile">
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
          <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
              <Clock size={12} style={{ color: 'var(--brand-cyan)' }} />
              <span className="bn">{lang === 'bn' ? 'শনি – বৃহস্পতি: সকাল ৯টা – রাত ৯টা' : 'Sat – Thu: 9 AM – 9 PM'}</span>
            </span>
            <span style={{ color: 'rgba(255,255,255,0.3)' }}>·</span>
            <span className="bn">{lang === 'bn' ? 'চট্টগ্রাম · ফেনী' : 'Chattogram · Feni'}</span>
          </div>
          <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
            <span style={{ color: 'rgba(255,255,255,0.42)' }} className="bn">{lang === 'bn' ? 'BMDC নিবন্ধিত' : 'BMDC Registered'}</span>
            <a href={`tel:${DIRECT_HELPLINE}`} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', color: 'var(--brand-cyan)', fontWeight: 700, textDecoration: 'none' }}>
        ...
        <div className="container">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 68 }}>
            ...
            <button onClick={() => go('home')} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }} aria-label="আলো আই কেয়ার">
              <img src={logoImg} alt="Alo Eye Care" style={{ height: 44, width: 'auto', display: 'block' }} />
              <div style={{ lineHeight: 1 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.3rem' }}>
                  {/* "আলো" — RED/ORANGE matching brochure exactly */}
                  <span style={{ fontFamily: 'Hind Siliguri, sans-serif', fontWeight: 700, fontSize: '1.4rem', color: '#E8201A', letterSpacing: '-0.01em' }}>আলো</span>
                  <span style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--brand-navy)', letterSpacing: '0.06em' }}>EYE CARE</span>
                </div>
                <div style={{ fontSize: '0.6rem', color: 'var(--brand-text-muted)', fontStyle: 'italic', marginTop: '1px' }}>
                  For your Eyes only
                </div>
              </div>
            </button>
              ...
              {NAV.map(item => {
                ...
                if (item.hasDrop) return (
                  <div key={item.section} style={{ position: 'relative' }}>
                    <button onClick={() => setDropOpen(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.4rem 0.75rem', background: active ? 'var(--brand-blue-pale)' : 'none', border: 'none', cursor: 'pointer', fontSize: '0.875rem', fontWeight: active ? 600 : 500, color: active ? 'var(--brand-blue)' : 'var(--brand-text)', borderRadius: '0.375rem', fontFamily: 'var(--font-body)', transition: 'all 0.15s' }} className="bn">
                      {lbl(item)}
                      <ChevronDown size={13} style={{ transform: dropOpen ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s' }} />
                    </button>
                    {dropOpen && (
                      <div style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, background: '#fff', border: '1px solid var(--brand-border)', borderRadius: '0.875rem', padding: '0.5rem', boxShadow: 'var(--shadow-card-hover)', minWidth: 260, zIndex: 60 }}>
                        {BRANCHES.map(b => (
                ...
                return (
                  <button key={item.section} onClick={() => go(item.section)} className="bn" style={{ padding: '0.4rem 0.75rem', background: active ? 'var(--brand-blue-pale)' : 'none', border: 'none', cursor: 'pointer', fontSize: '0.875rem', fontWeight: active ? 600 : 500, color: active ? 'var(--brand-blue)' : 'var(--brand-text)', borderRadius: '0.375rem', borderBottom: active ? '2px solid var(--brand-blue)' : '2px solid transparent', fontFamily: 'var(--font-body)', transition: 'all 0.15s' }}>
                    {lbl(item)}
                  </button>
                );
              })}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
              <button onClick={() => setLang(lang === 'bn' ? 'en' : 'bn')} style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.35rem 0.75rem', border: '1.5px solid var(--brand-border)', borderRadius: '999px', background: '#fff', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 700, color: 'var(--brand-blue)', transition: 'all 0.2s', fontFamily: 'var(--font-body)' }}
                ...
                <span style={{ color: 'var(--brand-border)', fontSize: '0.7rem' }}>|</span>
              ...
              <button onClick={() => go('booking')} className="desk-nav" style={{ display: 'none', alignItems: 'center', gap: '0.5rem', background: 'var(--brand-blue)', color: '#fff', border: 'none', borderRadius: '0.625rem', padding: '0.625rem 1.25rem', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', transition: 'all 0.2s', boxShadow: 'var(--shadow-btn)', fontFamily: 'var(--font-body)' }}>
                <Calendar size={15} />
                <span className="bn">{lang === 'bn' ? 'অ্যাপয়েন্টমেন্ট' : 'Book Now'}</span>
              </button>
        ...
        {menuOpen && (
          <div style={{ borderTop: '1px solid var(--brand-border)', background: '#fff', padding: '1rem' }}>
            ...
            <button onClick={() => go('booking')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', width: '100%', background: 'var(--brand-blue)', color: '#fff', border: 'none', borderRadius: '0.625rem', padding: '0.875rem', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
              <Calendar size={16} />
              <span className="bn">{lang === 'bn' ? 'অ্যাপয়েন্টমেন্ট বুক করুন' : 'Book Appointment'}</span>
            </button>
          </div>
        )}
      </header>
